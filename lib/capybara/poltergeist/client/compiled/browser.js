...
Poltergeist.Browser = class Browser {
  ...
  // afterwards.
  async select_file(page_id, id, files) {
    var node;
    node = this.node(page_id, id);
    await this.current_command.sendResponse(this.currentPage.beforeAction(node.id));
    await this.currentPage.uploadFile('[_poltergeist_selected]', ...files);
    return (await this.currentPage.afterAction(node.id));
  }
  ...
  async open_new_window() {
    ...
    page.urlBlacklist = this.page.urlBlacklist;
    page.urlWhitelist = this.page.urlWhitelist;
  ...
  set_headers(headers) {
    return this.currentPage.setCustomHeaders(headers).then(() => {
      return this.current_command.sendResponse(true);
    });
  }
  ...
  go_back() {
    ...
    return this.currentPage.goBack().then(() => {
  ...
  go_forward() {
    this.currentPage.state = 'wait_for_loading';
    return this.currentPage.goForward().then(() => {
      return this._waitForHistoryChange();
    });
  }
  ...
  refresh() {
    this.currentPage.state = 'wait_for_loading';
    return this.currentPage.reload().then(() => {
      return this._waitForHistoryChange();
    });
  }
