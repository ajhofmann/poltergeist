...
Poltergeist.WebPage = (function() {
  ...
  class WebPage {
    ...
    async uploadFile(selector, ...file_paths) {
      var eh;
      eh = (await this.currentFrame().$(selector));
      return (await eh.uploadFile(...file_paths));
    }
    ...
    async cookies() {
      return (await this.native().cookies());
    }

    async setCookie(...cookies) {
      return (await this.native().setCookie(...cookies));
    }

    async deleteCookie(...names) {
      var name;
      return (await this.native().deleteCookie(...((function() {
        var k, len2, results1;
        results1 = [];
    ...
    async clearCookies() {
      ...
      return (await this.native()._client.send('Network.clearBrowserCookies'));
    }
    ...
    setCustomHeaders(headers) {
      ...
      return (this._customHeaders['User-Agent'] ? this.setUserAgent(this._customHeaders['User-Agent']) : Promise.resolve()).then(() => {
        return this.native().setExtraHTTPHeaders(new Map(Object.entries(headers)));
      });
    }
    ...
    async removeTempHeadersForRedirect() {
      ...
      return (await this.setCustomHeaders(allHeaders));
    }

    async removeTempHeaders() {
      var allHeaders, name, ref2, value;
      allHeaders = this.getCustomHeaders();
